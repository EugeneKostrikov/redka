...
Redka.prototype.worker = function(queue, options){
  ...
  const w = worker.create(queueName, this.options.redis, options, this.prefix);
  this._workers[queueName] = w;

  //make sure there's a delayed jobs manager running
  this.delayedJobsManager.start();
  ...
  async.parallel([
    callback => {
      this.reporter.stop(function(err){
        console.log('reporter stopped ', err);
        callback();
      });
    },
    callback => {
      async.each(Object.keys(this._workers), (worker, next) => {
        ...
        console.log('workers stopped ', err);
        callback();
      });
    },
    callback => {
      this.delayedJobsManager.stop(function(err){
        console.log('delayer stopped ', err);
        callback();
      });
    }
  ], callback);
...
Redka.prototype.status = function(callback){
  const results = {};
  const that = this;
  let requested = 0, complete = 0;
  Object.keys(that._workers).forEach(function(k){
    const w = that._workers[k];
    requested++;
    that.client.multi()
...
Redka.prototype._reset = function(callback){
  ...
  keys.forEach(function(key){
    const worker = that._workers[key];
    worker.stats = {
      enqueued: 0,
      dequeued: 0,
      failed: 0,
