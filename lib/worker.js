...
Worker.prototype.poll = function(){
  ...
  this.dequeue((err, job) => {
    ...
    this.timeout(job, stopHeartbeat);
...
Worker.prototype.timeout = function(job, stopHeartbeat){
  if (!this.to) return;
  debug('setting timeout on job', job.id, this.to);
...
Worker.prototype.handleError = function(err, callback){
  ...
  //TODO: handle error when there's no callback. Bubble the error up as event from redka instance
};
