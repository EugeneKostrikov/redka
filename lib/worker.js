...
Worker.prototype.enqueue = function(name, params, callback){
  var job = Job.create(this.queue, name, params);
  this.client.hmset(job.id, job.serialize(), (err) => {
    if (err) return this.handleError(err, callback);
    this.client.lpush(this.lists.pending, job.id, (err) => {
      if (err) return this.handleError(err, callback);
      callback && callback();
    });
  });
...
Worker.prototype.dequeue = function(callback){
  this.pollingClient.brpoplpush(this.lists.pending, this.lists.progress, 10, (err, item) => {
